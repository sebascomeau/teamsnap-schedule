...
export const loader = async ({ params, context }: LoaderFunctionArgs) => {
  ...
  const teamAll = teamService.getTeamAll();

  // undefined means all teams
  const team = params.teamId
    ? await teamService.getTeam(Number.parseInt(params.teamId))
    : teamAll;

  if (team === null) {
    // not found
    throw json("Team Not Found", { status: 404 });
  }
  ...
  const searchEventsTeamIds =
    team.id === teamAll.id
      ? (await teamService.getRootDivisionTeams()).map(({ id }) => id)
      : [team.id];
